...
const htmlParaPrecioPromedio = (restaurantData) => {
    return '<div class="s327yx-1 hKRjCv s327yx-2 kYqztZ">' + svg$ + '<span color="#7f7304" class="sc-10jqr7t-0 s327yx-0 fjmNsw s327yx-2 kYqztZ">' + restaurantData.precioPromedio + '</span></div>';
}
...
var restaurantData = ""
getRestaurantData().then(data => restaurantData = data);

const getShippingAmount = (restaurantData) => {
    return restaurantData.shippingAmount ? `Envio $${restaurantData.shippingAmount}` : 'Envio Gratis';
}


const starsDiv = (startsNumber) => {
            ...
            <div class="sc-1f3rtp4-4 cvxEpM">
                ...
                <div class="jgqhzl-1" style="width: 270px;">
                    ...
                    <div>
                        ...
                        <span> </span>
                        <span>${getShippingAmount(restaurant)}</span>
                    </div>
                    <div>
                        <span>${starsDiv(restaurant.service)}</span>
                        <span></span>
                        <span>${htmlParaPrecioPromedio(restaurant)}</span>
                    <div>
                    </div>
                </div>
            </div>
...
const renderRestarantsData = (restaurantData) => {
    ...
    let centerColumn = document.getElementsByClassName("sc-60supj-0 iRsYvI")[0];
    wrap(centerColumn, divPadreDeLas2Columnas)
    ...
    restaurantsContainer = document.getElementById("precioPromedioColumn");

    //Ordeno antes de renderizar
    restaurantData = restaurantData.sort((a, b) => { return a.precioPromedio - b.precioPromedio });
    restaurantData.map(restaurant => restaurantsContainer.insertAdjacentHTML("beforeEnd", renderRestaurant(restaurant)))
}


const testFunction = (restaurant) => {
    restaurantsContainer = document.getElementsByClassName("sc-60supj-0 iRsYvI")[0];
    restaurantsContainer.innerHTML = "";
    restaurantsContainer.insertAdjacentHTML("beforeEnd", renderRestaurant(restaurant));
}
